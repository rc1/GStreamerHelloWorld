      ...
      char *argv[])
{
  ...
  /* Piplines */
  GstElement *pipeline;
  //low_pipeline = gst_pipeline_new ("low_pipeline");
  //high_pipeline = gst_pipeline_new ("high_pipeline");
  pipeline = gst_pipeline_new ("pipline");

  /* Elements */
  GstElement *low_source, *high_source;
  ...
  if (!pipeline || !low_source || !high_source || !mixer || !conv || !resample || !resample || !sink ) {
  ...
  g_object_set (G_OBJECT (low_source), "volume", 0.3, "freq", 440.0, "timestamp-offset", 1000000000, NULL);
  ...
  gst_bin_add_many (GST_BIN (pipeline), low_source, high_source, mixer, conv, resample, sink, NULL);
